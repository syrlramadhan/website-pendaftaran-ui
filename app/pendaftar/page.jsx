...
const Pendaftar = () => {
  ...
  const exportToExcel = async () => {
    ...
    const dataToExport = registrants.map((registrant, index) => ({
  ...
  const downloadAsZip = async () => {
    try {
      ...
      const zip = new JSZip();
      const documentsFolder = zip.folder("bukti_tf");
      const token = Cookies.get("adminToken");

      const fetchPromises = registrants.map(async (registrant, index) => {
        if (registrant["bukti-transfer"] && registrant["bukti-transfer"] !== "no data") {
          try {
            ...
            const decodedUrl = decodeURIComponent(registrant["bukti-transfer"]);
            const originalFileName = decodedUrl.split("/").pop();
            documentsFolder.file(originalFileName, blob);
          } catch (err) {
      ...
      link.download = `bukti_tf_${new Date().toISOString().replace(/[:.]/g, "-")}.zip`;
      link.click();
    } catch (err) {
